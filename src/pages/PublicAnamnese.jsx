...
export default function PublicAnamnese() {
  ...
  const rv = answers.ri
  const showCrisis = step === 5 && (rv === 'Sim – estou em risco' || rv === 'Não sei' || rv === 'Prefiro conversar')
  ...
  const submit = async () => {
    setBusy(true)
    const risk = ['Sim – estou em risco','Não sei'].includes(answers.ri)
  ...
  if (done) return (
    <div className="auth-shell">
      <div className="auth-card" style={{textAlign:'center'}}>
        ...
        <p className="sub">Suas respostas foram entregues com segurança a sua terapeuta.<br/><br/>Em caso de crise: CVV 188 · SAMU 192</p>
      </div>
    </div>
  )
  ...
  return (
    <div style={{minHeight:'100vh',background:'var(--bg)'}}>
      ...
      <div className="anm-wrap">
        ...
        {step===0 && <div className="consent-box">Este formulário é <strong>estritamente confidencial</strong> e protegido pela LGPD. Responda apenas o que se sentir confortável. <strong>⚠️ Risco imediato: SAMU 192 · CVV 188</strong></div>}
        {showCrisis && <div className="crisis-box"><strong>⚠️ Atenção imediata</strong>Ligue agora: CVV 188 · SAMU 192 · UPA mais próxima.<br/>Você pode continuar — a terapeuta será notificada com prioridade.</div>}
        ...
        <div className="anm-nav">
          ...
          <div style={{display:'flex',gap:8}}>
            ...
            {step < ANM_SECTIONS.length - 1
              ? <button className="btn btn-p" onClick={() => setStep(step+1)}>Próximo →</button>
